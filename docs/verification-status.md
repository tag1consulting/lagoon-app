...
- **Backups, retrieval, and environment variables** — implemented; a device test against a live instance surfaced a backup-retrieval UX bug (fixed), so this area is not yet re-confirmed. Treat as unverified until confirmed the same way login/deployments/tasks were.
