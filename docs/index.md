...
hero_tagline: "A mobile client for the Lagoon application delivery platform. Browse projects and environments, watch deployments and tasks with their build logs, and trigger deploys — across multiple Lagoon instances."
---
...
Lagoon Mobile is a monitor-and-operate client for [Lagoon](https://github.com/uselagoon/lagoon): browse projects and environments, watch deployments and tasks with their build logs, and trigger `deployEnvironmentLatest`, `cancelDeployment`, `invokeRegisteredTask`, and raw `addTask` — all scoped to whichever Lagoon instance you're currently connected to. Android-first, with iOS builds from the same codebase.
