...
Backups are browse-and-retrieve only: `addBackup`/`updateRestore` exist in Lagoon's schema, but they're for the backup agent to register that a backup already happened, not a "trigger a backup now" action, so the app never creates backups. Lagoon cannot automatically restore a backup into a live environment — triggering a retrieval (`addRestore` with `execute: true`) only makes the backup's files available to download; the user applies them manually. Because nothing is overwritten, the confirm sheet is non-destructive, unlike every other mutating action in the app. Deleting a backup record and downloading its file (only available once retrieval has completed, and only on Lagoon ≥2.29) are the other two actions on each row.
